...
import type { TischtennisSpiel } from '@/lib/click-tt'

interface Props {
  kommend: TischtennisSpiel[]
  vergangen: TischtennisSpiel[]
}

export default function TischtennisSpieleClient({ kommend, vergangen }: Props) {
  const [activeFilters, setActiveFilters] = useState<Set<string>>(new Set())

  /* ── Collect all unique mannschaft values ── */
  const allTeams = useMemo(() => {
    const set = new Set<string>()
    ;[...kommend, ...vergangen].forEach(s => set.add(s.mannschaft))
  ...
  function matchesFilter(s: TischtennisSpiel): boolean {
    if (activeFilters.size === 0) return true
    return activeFilters.has(s.mannschaft)
  }
      ...
      {vergangen.length > 0 && (
        <div>
          ...
          <div className="mt-4 text-right">
            <a
              href="https://ttvn.click-tt.de/cgi-bin/WebObjects/nuLigaTTDE.woa/wa/clubMeetings?club=5504"
              target="_blank"
            ...
            >
              Alle Ergebnisse auf click-tt <ExternalLink size={11} />
            </a>
          </div>
        </div>
      )}
...
function SpielRow({ spiel }: { spiel: TischtennisSpiel }) {
  ...
  return (
    <div className="bg-[#f5f5f0] p-6 md:p-8 flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
      ...
      <div className="bg-[#0a0a0a] text-[#f5f5f0] p-4 text-center shrink-0 w-20">
        <div className="font-display text-3xl leading-none">{format(date, 'd')}</div>
        <div className="text-[10px] tracking-[0.2em] uppercase text-[#f5f5f0]/60 mt-1">
          {format(date, 'MMM', { locale: de })}
        </div>
        <div className="text-[10px] text-[#f5f5f0]/40">{format(date, 'yyyy')}</div>
      </div>
...
function SpielRowKompakt({ spiel }: { spiel: TischtennisSpiel }) {
  return (
